...
- I get more done. Sometimes *a lot* and, other times, just more. I'm measuring in results, which include usable software artifacts, digital artifacts (documents, images, sounds, etc.), and problems solved; not lines of code, or amount of words.
...
- I get to discuss ideas with a responsive rubber duck, which can be quite helpful. The rubber duck is sometimes very wrong but, if you're aware of this, and keep your guard, that doesn't matter that much. What I mean by "keep your guard" is you must always validate what comes out of the ducks' mouth. How to do this isn't always easy, but nothing in life is.
  - Subnote: some colleagues and myself are noticing that, in the specific case of Claude Code, the agent seems to be trying to validate what *we* say. For example, we can say "Connect to X which is the reader" and the agent's thinking output will say "Validating that X is the reader." It would be very funny if this was because enough posts from us human complaining about hallucinations ended up training the agents to protect against ours.
...
- I've found that the harness is typically more important (in terms of outcome quality) than the model (this is in the context of writing agents).

- The best approach that works for me is usually: use the models to help me write more tools, but then eventually only use the deterministic tools when investigating or troubleshooting. An exception is I run the output to tools by a model as getting a second opinion, but I only do this when I'm an expert in the problem domain. Otherwise, it's too risky to get fooled by a confidently-stated hallucination.
...
- It seems "AI == LLM" now. They're great, but sometimes people use them for the wrong task. Say you want to go through a lot of metrics and do an open-ended investigation to see if anything stands out. Something very generic. I'd start with good old correlation algorithms and then only involve the LLM to analyze those results.


*Addendum: challenges*

- LLMs are usually too verbose. They can be used to summarize text, but the more you press in that direction, the higher the risk of missing or wrong information. Codebases can get bloated quickly, especially with useless comments. Some would argue the comments are using for agents, but I'm not yet convinced of that, since they seem to do a pretty decent job already at figuring out what poorly documented codebases do anyway, and the more context they need to deal with, the higher the risk of things going wrong.
...
- Agents can be, even within the same session, both very confident yet wrong, and very right yet quick to apologize and change course (to the wrong one, that is).
